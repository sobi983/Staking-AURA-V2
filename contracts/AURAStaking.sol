...
import "./AURAStakingInterface.sol";
...
contract AURAStaking is Ownable, Pausable, ReentrancyGuard, UserStats, Reward, Staking  {
    ...
    function CurrentAPR() public override view returns(uint256){
         return APR;
    }
    ...
    function ChangeMinStakingAmount(uint256 _MinAmount) public override onlyOwner returns(bool){
        minStakingAmount = _MinAmount;
        return true;
    }

    function MinStakingAmount()public override view returns(uint256){
    ...
    function AllocationOfRewards() public override view returns(uint256){
        return allocatedReward;
    }

    function IncreaseAllocationReward(uint256 _amount)public onlyOwner override returns(bool){
        allocatedReward += _amount;
        token.transferFrom(msg.sender, address(this), _amount);
        return true;
    }

    function DecreaseAllocationReward(uint256 _amount)public onlyOwner override returns(bool){
        allocatedReward -= _amount;
        token.transfer(msg.sender,_amount);
        return true;
    }

    function TransferAllocatedRewardFromContractToOwner(uint256 _amount)public onlyOwner override returns(bool){
        token.transfer(msg.sender,_amount);
        return true;
    }
    ...
    function TotalWithdrawal(address _user) public override view returns(uint256){
        return user[_user].totalWithdrawal;
    }

    function TotalPendingReward(address _user) public override view returns(uint256){
        return user[_user].totalPendingReward;

    }

    function TotalAccumulatedReward(address _user) public override view returns(uint256){
        return user[_user].totalAccumulatedReward;
    }

    function CalculateReward(uint256 _amount, uint256 _time) public override view returns(uint256){
        return ((_time * _amount * APR)/365 days)/100;
    }
    
    //Number of users staked in the Staking Pool
    function TotalDelegates() public override view returns(uint256){
        return totalDelegates;
    }

    //The user's Current status of the stakes
    function CurrentlyStaked(address _user) public override view returns(uint256){
        return user[_user].currentlyStaked;
    }
    
    //The user's current status of the stakes is whether active?
    function HasStake(address _user) public override view returns(bool){
        if(user[_user].currentlyStaked == 0) {return false;}
        return true;
    }

    //Total stakes in the pool status 
    function TotalStakedInPool()public override view returns(uint256){
        return totalStakedInPool;
    }
    
    //pause it when finished
    function Stake(uint256 amount, uint256 time) public whenNotPaused() override returns(bool){
        uint256 RewardOFUser = CalculateReward(amount * 10**18,time);
        if(token.balanceOf(_msgSender()) < (10**18 * amount) || amount*10**18 < minStakingAmount) revert InsufficientBalance();
        if(time < 1209600 || time > 31536000) revert TimeIncorrect();
        if(user[_msgSender()].currentlyStaked == 0){
            unchecked{
                ++totalDelegates;
            }}  
        if(AllocationOfRewards() < RewardOFUser) revert InsufficientRewardsInPool();
    ...
    function _stake(uint256 _amount, uint256 _time,uint256 _reward) internal {
    ...
    function ReStake(uint256 amount,uint256 time) public whenNotPaused() override returns(bool){
        uint256 RewardOFUser = user[_msgSender()].totalPendingReward/10**18 + CalculateReward((user[_msgSender()].currentlyStaked + amount* 10**18) ,time);
        if(token.balanceOf(_msgSender()) < (10**18 * amount) || amount*10**18 < minStakingAmount) revert InsufficientBalance();
        if(time < 1209600 || time > 31536000) revert TimeIncorrect();
        if(user[_msgSender()].currentlyStaked == 0){
            unchecked{
    ...
    function _reStake(uint256 _amount,uint256 _time, uint256 _reward) internal {
    ...
    function _withdraw() internal {
        ...
        if(user[_msgSender()].totalPendingReward == 0 && user[_msgSender()].currentlyStaked == 0){
...
//Testing purpose will be tested further in HArdhat...
    uint256 Time;
