    ...
    Staking
{
        ...
        returns (bool)
    {
        ...
        if (
            token.balanceOf(_msgSender()) < (10**18 * amount) ||
            amount * 10**18 < minStakingAmount
    ...
    function ReStake(uint256 amount, uint256 time)
        ...
        returns (bool)
    {
        uint256 RewardOFUser = user[_msgSender()].totalPendingReward /
            10**18 +
            CalculateReward(
                (user[_msgSender()].currentlyStaked + amount * 10**18),
                time
            );
        if (
            token.balanceOf(_msgSender()) < (10**18 * amount) ||
