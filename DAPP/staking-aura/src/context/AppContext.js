...
const AppState = ({ children }) => {
	...
	const [FormatedBalance, setFormatedBalance] = useState(0);
	...
	async function getData() {
		...
		console.log(format, "Non Formated balance");
		Math.abs(Number(formated)) >= 1.0e9
			? setFormatedBalance(Math.abs(Number(formated)) / 1.0e9 + "B")
			: // Six Zeroes for Millions
			Math.abs(Number(formated)) >= 1.0e6
			? setFormatedBalance(Math.abs(Number(formated)) / 1.0e6 + "M")
			: // Three Zeroes for Thousands
			Math.abs(Number(formated)) >= 1.0e3
			? setFormatedBalance(Math.abs(Number(formated)) / 1.0e3 + "K")
			: setFormatedBalance(Math.abs(Number(formated)));
		// setFormatedBalance(formated);
		console.log(formated, "formated balance");
